package com.adaptive.common.dao;
...
public abstract class GenericDAOHibernate<T, ID extends Serializable>
	...
	//end added by susanto
	
	// start added by robbin
	
	  public Connection getConnection() { return getSession().connection(); }
	...
	//start added by malik
	public Integer countUniqueKey(String valueName, String columnName, T entity) {
		int result = 1;
		
		try {
			
			Criteria criteria = getSession().createCriteria(entity.getClass());
			criteria.add(Restrictions.eq(columnName, valueName).ignoreCase());
